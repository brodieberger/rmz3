...
// 00 00 nn nn
NON_MATCH static void GameLoop_NewGame(struct GameState* g) {
#if MODERN
  (&(g->save))->gamemode = 0;
...
NON_MATCH static void GameLoop_Overworld(struct GameState* p) {
#if MODERN
  bool8 isPaused;
  bool32 escape;
...
*/
NON_MATCH static void GameLoop_ChangeMap(struct GameState* p) {
#if MODERN
  UpdateStoryFlag();
