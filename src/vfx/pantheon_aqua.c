...
struct Entity* FUN_080ba81c(struct Coord* c, bool8 isRight, motion_t m, u32 val) {
...
struct Entity* FUN_080ba940(struct Coord* c, u8 n, u8 r2) {
  struct VFXUnkCommon* p = (struct VFXUnkCommon*)AllocEntityFirst(gVFXHeaderPtr);
  if (p != NULL) {
    (p->s).taskCol = 1;
    INIT_VFX_ROUTINE(p, VFX_P_AQUA);
...
static void FUN_080bb7c4(struct Entity* p) {
  ...
  switch (p->mode[2]) {
    case 0: {
      SetMotion(p, MOTION(SM039_PANTHEON_AQUA, 12));
      p->work[3] = 0;
      p->mode[2]++;
      FALLTHROUGH;
    }
