...
NON_MATCH static void inIdle(struct Zero* z) {
  ...
  if ((status & BODY_STATUS_CHAT) && (key & ZERO_INPUT_PRESS_DPAD_UP)) {
    (z->s).mode[1] = ZERO_TALK, (z->s).mode[2] = 0, (z->s).mode[3] = 0;
    return;
  }
  if ((status & BODY_STATUS_TELEPORTAL) && (key & ZERO_INPUT_PRESS_DPAD_UP)) {
    (z->s).mode[1] = ZERO_TELEPORT, (z->s).mode[2] = 0, (z->s).mode[3] = 1;
    return;
  }
  ...
  if (key & (DPAD_RIGHT | DPAD_LEFT)) {
    (z->s).mode[2] = GROUND_WALK, (z->s).mode[3] = 0;
    return;
  }
  ...
  if ((key & DPAD_DOWN) && (TryLadderDown(z, gZeroRanges, FALSE) == 2)) {
    (z->s).mode[1] = ZERO_LADDER, (z->s).mode[2] = LADDER_DOWN, (z->s).mode[3] = 0;
    return;
  }
  if ((key & DPAD_UP) && (TryLadderUp(z, gZeroRanges, FALSE) != 0)) {
    (z->s).mode[1] = ZERO_LADDER, (z->s).mode[2] = LADDER_UP, (z->s).mode[3] = 0;
  }
#else
...
DONE:
  if (((*key) & DPAD_UP) && TryLadderUp(z, &gZeroRanges[0], FALSE)) {
    (z->s).mode[1] = ZERO_LADDER, (z->s).mode[2] = 0, (z->s).mode[3] = 0;
    return;
  }
  ...
  if ((z->s).mode[2] == 2) {
    if (CanWallSlide(z, &gZeroRanges[z->posture], FALSE)) {
      (z->s).mode[1] = ZERO_WALL, (z->s).mode[2] = 0;
    }
  }
#else
